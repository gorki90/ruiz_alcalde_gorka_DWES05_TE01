...
<head>
    ...
    <title>Modifica tu Build</title>
    <link rel="stylesheet" href="{{ asset('css/app.css') }}">
</head>
<body>
    <h2>Modificar Build</h2>
    <form class="form" action="{{ route('updateBuild_ruta', ['id' => $id]) }}" method="POST">
        @csrf 
        <input type="hidden" name="id_build" value="{{ $id }}">
        <label for="nombre_build">Nombre de la Build:</label>
        <input type="text" id="nombre_build" name="nombre_build"><br><br>
        
        <label for="descripcion_build">Descripción de la Build:</label>
        <textarea id="descripcion_build" name="descripcion_build"></textarea><br><br>
